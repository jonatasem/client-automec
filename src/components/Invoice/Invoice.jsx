...
const Invoice = () => {
    ...
    if (salesLoading || clientsLoading) return <div className='loading'>Carregando...</div>; 
    if (salesError) return <div className='error-loading'>Erro ao buscar venda: {salesError.message}</div>; 
    if (clientsError) return <div className='error-loading'>Erro ao buscar cliente: {clientsError.message}</div>; 
    ...
    const generatePDF = () => {
        ...
        html2canvas(invoiceRef.current, { scale: 2 }).then(canvas => {
            ...
            const imgWidth = pdf.internal.pageSize.getWidth(); // Largura da página
            ...
            // Se a altura exceder a altura da página, adicione novas páginas
            while (heightLeft >= 0) {
    ...
    return (
        <section className="container-invoice" ref={invoiceRef}>
            <div className="layout-invoice">
                ...
                <article className="invoice-header">
                    ...
                    <p className='data-invoice'>Data de Faturamento: {new Date(sale.data_faturamento).toLocaleDateString()}</p>
                ...
                <article className="invoice-footer">
                    <p><strong>Valor Total: R$ {sale.valor_total.toFixed(2)}</strong></p>
